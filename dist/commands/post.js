...
const PostCommand = (bot) => {
    bot.command("post", async (ctx) => {
        ...
        const refinement = await prisma_1.prisma.refinement.findUnique({
            where: { userId },
        });
        ...
        await ctx.reply(optionsText, telegraf_1.Markup.inlineKeyboard([
            [telegraf_1.Markup.button.callback("✏️ Edit Preferences", "edit_pref_post")],
            [
                telegraf_1.Markup.button.callback("✅ Preferences are fine", "confirm_pref_post"),
            ],
        ]));
    });
    ...
    bot.on("text", async (ctx, next) => {
        ...
        if (ctx.chat?.type !== "private")
            return next(); // only handle private messages for post flow
        if (!waitingForPost[userId])
            return next();
        ...
        const placeholder = await ctx.reply("⏳ Refining your text, please wait...");
        ...
        try {
            ...
            const result = await (0, gemini_1.refineText)(message, {
            ...
            if (result.fallback) {
                ...
                if (!process.env.GEMINI_API_KEY) {
                    await ctx.reply("⚠️ Gemini is not configured (GEMINI_API_KEY missing). The text was returned unchanged.");
                }
                else {
                    // Non-config / network error; log and inform user
                    await ctx.reply(`⚠️ Gemini returned an error (${result.error}${result.status ? ` - ${result.status}` : ``}). The text was returned unchanged.`);
                }
            }
            refined = result.text;
        }
        ...
        try {
            await ctx.telegram.editMessageText(ctx.chat.id, placeholder.message_id, undefined, `Refined message:\n\n${refined}`);
        }
        catch (e) {
