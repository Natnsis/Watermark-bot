...
async function refineText(input, options) {
    // If there's no GEMINI API key, return the message unchanged as a fallback.
    if (!API_KEY) {
        console.warn("GEMINI_API_KEY is not set — returning original input as fallback.");
    ...
    // Grammar rules
    if (grammar)
        rules.push("Fix grammar and spelling.");
    else
        rules.push("Do not modify grammar or spelling.");
    // Tone and emoji rules
    if (professional && funny) {
        // Both requested — keep professional but add a light humorous tone without emojis.
        rules.push("Apply a professional tone and formal register; you may add mild humor but do NOT include any emojis.");
    }
    else if (professional) {
        rules.push("Apply professional tone and no emojis; prefer formal wording.");
    }
    else if (funny) {
        rules.push("Add humor and lively tone; emojis are allowed and encouraged where appropriate.");
    }
    else {
    ...
    try {
        ...
        if (!generated) {
            console.warn("Gemini response did not contain any text - returning original");
            return { text: input, fallback: true, error: "no-output" };
        }
        // `generated` could be a string or complex object; coerce to string.
        let text = typeof generated === "string" ? generated : JSON.stringify(generated);
        text = text.trim();
        ...
        if (Array.isArray(sentenceMatches) && sentenceMatches.length > 1) {
            ...
            while (sentenceMatches.length > 0 &&
        ...
        text = text.replace(/^(no\s+(grammar|professional|professionalism|funny|emojis|emoji)(\s|$).*)/i, "");
        text = text.replace(/(no\s+(grammar|professional|professionalism|funny|emojis|emoji)(\s|$).*)$/i, "");
        text = text.trim();
