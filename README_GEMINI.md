...
- Note: The wrapper now authenticates using the official `@google/genai` client with `GEMINI_API_KEY`. For production, prefer a service account and `google-auth-library` to fetch short-lived tokens automatically.
...
- The wrapper now uses the official client; it authenticates with `GEMINI_API_KEY`.
